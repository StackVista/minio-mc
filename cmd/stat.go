...
func (v bucketInfoMessage) String() string {
	...
	if info.Encryption.Algorithm != "" {
		...
		fmt.Fprintf(&b, "%s", console.Colorize("Key", "\n\tAlgorithm: "))
		fmt.Fprintf(&b, "%s", console.Colorize("Value", info.Encryption.Algorithm))
		fmt.Fprintf(&b, "%s", console.Colorize("Key", "\n\tKey ID: "))
		fmt.Fprintf(&b, "%s", console.Colorize("Value", info.Encryption.KeyID))
		fmt.Fprintln(&b)
	}
	fmt.Fprintf(&b, "%2s%s", placeHolder, "Versioning: ")
	if info.Versioning.Status == "" {
		fmt.Fprintf(&b, "%s", console.Colorize("Unset", "Un-versioned"))
	} else {
		fmt.Fprintf(&b, "%s", console.Colorize("Set", info.Versioning.Status))
	}
	fmt.Fprintln(&b)
	...
	if info.Locking.Mode != "" {
		...
		fmt.Fprintf(&b, "%s", console.Colorize("Value", info.Locking.Mode))
		fmt.Fprintln(&b)
		fmt.Fprintf(&b, "%4s%s", placeHolder, "Retention Until Date: ")
		fmt.Fprintf(&b, "%s", console.Colorize("Value", info.Locking.Validity))
		fmt.Fprintln(&b)
	}
	if len(info.Notification.Config.TopicConfigs) > 0 {
		fmt.Fprintf(&b, "%2s%s", placeHolder, "Notification: ")
		fmt.Fprintf(&b, "%s", console.Colorize("Set", "Set"))
		fmt.Fprintln(&b)
	}
	if info.Replication.Enabled {
		fmt.Fprintf(&b, "%2s%s", placeHolder, "Replication: ")
		fmt.Fprintf(&b, "%s", console.Colorize("Set", "Enabled"))
		fmt.Fprintln(&b)
	}
	fmt.Fprintf(&b, "%2s%s", placeHolder, "Location: ")
	fmt.Fprintf(&b, "%s", console.Colorize("Generic", info.Location))
	fmt.Fprintln(&b)
	fmt.Fprintf(&b, "%2s%s", placeHolder, "Policy: ")
	if info.Policy.Type == "none" {
		fmt.Fprintf(&b, "%s", console.Colorize("UnSet", info.Policy.Type))
	} else {
		fmt.Fprintf(&b, "%s", console.Colorize("Set", info.Policy.Type))
	}
	fmt.Fprintln(&b)
	if info.Tags() != "" {
		fmt.Fprintf(&b, "%2s%s", placeHolder, "Tagging: ")
		fmt.Fprintf(&b, "%s", console.Colorize("Generic", info.Tags()))
		fmt.Fprintln(&b)
	}
